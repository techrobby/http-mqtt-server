...
import com.gk.server.http.message.FullEncodedResponse;
...
public class ResponseEncoder extends ChannelOutboundHandlerAdapter
{
	...
	public void write(ChannelHandlerContext ctx, Object msg, ChannelPromise promise) throws Exception
	{
		 ...
		 * CookieDecoder.decode(cookieString); if (!cookies.isEmpty()) { // Reset the cookies if necessary. for (Cookie cookie : cookies) { if
		 * (cookie.getName().equals(SESSION_COOKIE_NAME)) { hasSessionId = true; } httpResponse.headers().add(SET_COOKIE, ServerCookieEncoder.encode(cookie)); } } } if
		 * (!hasSessionId) { httpResponse.headers().add( SET_COOKIE, ServerCookieEncoder.encode(SESSION_COOKIE_NAME, nextSessionId())); }
		 */
		...
		{
			pendingResponses.put(encodedResponse.getRequest().getOrderNumber(), httpResponse);
